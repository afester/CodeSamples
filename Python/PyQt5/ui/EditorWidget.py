...
class FindWidget(QWidget):
    ...
    def find(self, isUpwards):
        ...
        if isUpwards:
            foundCrsr = doc.find(self.pattern, self.startPos, QTextDocument.FindFlag.FindBackward)
        else:
...
class EditorWidget(QWidget):
    ...
    LOG = logging.getLogger('EditorWidget')
    ...
    def load(self, notepad, pageId):
        self.LOG.debug('Loading page "{}" from notepad "{}"'.format(pageId, notepad.getName()))
    ...
    def updateWindowTitle(self, flag):
        ...
        self.titleChanged.emit('{}{}: {}'.format('*' if flag else '', self.page.notepad.getRootpath(),
                                                 self.page.getName()))
    ...
    def save(self):
        if self.editView.document().isModified():
            self.LOG.debug('Page "{}" from notepad "{}" has been modified, saving page'.format(
                self.page.getName(), self.page.notepad.getName()))
            self.page.save()
