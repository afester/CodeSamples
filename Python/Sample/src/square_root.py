...
def sqrt_heron(value: float) -> float:
    ...
    while abs(result - prev_result) > sys.float_info.epsilon:
        prev_result = result
        result = 0.5*(prev_result + value / prev_result)
        step += 1
    print(f"{step} iterations.")
    return result
...
def main(argv):
    ...
    # print(f"sqrt(2)={sqrtHeron(2)}")

    # print(f"sqrt(3)={sqrt(3)}")
    # print(f"sqrt(3)={sqrtHeron(3)}")
