...
public class GraphCanvas extends Canvas {
    ...
    private void drawGraph(Graph theGraph) {
       ...
       // theGraph.setT(this.t);
        double scalePerPixel = 1/gp.getScale();
        for (double x = gp.getFromX(); x < gp.getToX(); x += scalePerPixel) {
           try {
              ...
              if (!Double.isNaN(y)) {
                  ...
                  System.err.println("y=" + y + ", ypos=" + ypos);
                 if (move) {
                    move = false;
    ...
    /**
     * @param y tick position in model coordinates
     */
    private void drawYTick(double y) {
     ...
     /**
      * @param x tick position in model coordinates
      */
